...
import { ATTRIBUTE_METADATA_QUERY, PRODUCT_SEARCH_QUERY, REFINE_PRODUCT_QUERY } from "./queries";
...
const getProductSearch = async ({
...
}: ProductSearchQuery & ClientProps): Promise<ProductSearchResponse["data"]> => {
    const variables = {
        phrase,
        pageSize,
        currentPage,
    ...
    magentoStorefrontEvtPublish?.searchRequestSent && magentoStorefrontEvtPublish.searchRequestSent(SEARCH_UNIT_ID);
    ...
    // ======  initialize data collection =====
    updateSearchResultsCtx(SEARCH_UNIT_ID, searchRequestId, results?.data?.productSearch);
    ...
    if (categorySearch) {
        ...
        magentoStorefrontEvtPublish?.searchResultsView && magentoStorefrontEvtPublish.searchResultsView(SEARCH_UNIT_ID);
    }
    // ======  end of data collection =====
