...
import { Facet as FacetType, FacetFilter, PriceFacet } from "../types/interface";

export const useScalarFacet = (facet: FacetType | PriceFacet) => {
    const searchCtx = useSearch();
    const filter = searchCtx?.filters?.find((e: FacetFilter) => e.attribute === facet.attribute);
    ...
    const onChange = (value: string, selected?: boolean) => {
        ...
        newFilter.in = selected ? [...currentFilterIn, value] : filter.in?.filter((e: string) => e !== value);
