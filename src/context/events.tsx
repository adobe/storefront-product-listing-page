...
const createProducts = (items: ProductSearchResponse["data"]["productSearch"]["items"]): SearchResultProduct[] => {
    if (!items) {
        return [];
    }
    ...
    const products: SearchResultProduct[] = items.map((item, index) => ({
        ...
        imageUrl: item?.productView?.images?.length ? (item?.productView?.images[0].url ?? "") : "",
        price:
...
const createFacets = (items: ProductSearchResponse["data"]["productSearch"]["facets"]): SearchFacet[] => {
    if (!items) {
        return [];
    }
