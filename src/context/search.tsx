...
import { FacetFilter, ProductSearchSortInput, SearchClauseInput } from "../types/interface";
import { SEARCH_SORT_DEFAULT } from "../utils/constants";
import { addUrlFilter, getValueFromUrl, removeAllUrlFilters, removeUrlFilter } from "../utils/handleUrlFilters";
...
const SearchProvider: FunctionComponent = ({ children }) => {
    ...
    const sortDefault = graphQLSort ? (graphQLSort as ProductSearchSortInput[]) : SEARCH_SORT_DEFAULT; // default to "relevance" sort

    const [phrase, setPhrase] = useState<string>(phraseFromUrl);
    const [categoryPath, setCategoryPath] = useState<string>("");
    const [filters, setFilters] = useState<SearchClauseInput[]>([]);
    const [categoryNames, setCategoryNames] = useState<{ name: string; value: string; attribute: string }[]>([]);
    ...
    const updateFilterOptions = (facetFilter: SearchClauseInput, option: string) => {
