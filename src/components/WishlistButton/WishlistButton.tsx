...
export const WishlistButton: FunctionComponent<WishlistButtonProps> = ({
  type,
  productSku,
}: WishlistButtonProps) => {
  const { isAuthorized, wishlist, addItemToWishlist, removeItemFromWishlist } =
    useWishlist();
  const wishlistItemStatus: WishlistItem | undefined =
    wishlist?.items_v2?.items.filter((ws) => ws.product.sku === productSku)[0];
