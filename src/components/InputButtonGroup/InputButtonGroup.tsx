    ...
    const formatLabel = (title: string, bucket: Bucket) => {
        if (bucket.__typename === "RangeBucket") {
            ...
            const label = `${currencySymbol}${
                bucket?.from && parseFloat(currencyRate) * parseInt(bucket.from.toFixed(0), 10)
                    ? (parseFloat(currencyRate) * parseInt(bucket.from.toFixed(0), 10)).toFixed(2)
                    : 0
            }${
                bucket?.to && parseFloat(currencyRate) * parseInt(bucket.to.toFixed(0), 10)
                    ? ` - ${currencySymbol}${(parseFloat(currencyRate) * parseInt(bucket.to.toFixed(0), 10)).toFixed(
                          2,
                      )}`
    ...
    return (
        <div className="ds-sdk-input pt-md">
            {inputGroupTitleSlot ? (
                ...
                <label className="ds-sdk-input__label text-base font-normal text-gray-900">{title}</label>
            )}
                    ...
                    {buckets.slice(0, numberOfOptions).map((option) => {
                        ...
                        return (
                            <LabelledInput
                                ...
                                onChange={(e: ChangeEvent<HTMLInputElement>) => onInputChange(option.title, e)}
                                type={type}
                            />
                        );
                    })}
