...
import { Product, ProductViewMedia, RedirectRouteFunc, RefinedProduct } from "../../types/interface";
import { SEARCH_UNIT_ID } from "../../utils/constants";
import { generateOptimizedImages, getProductImageURLs } from "../../utils/getProductImage";
...
export const ProductItem: FunctionComponent<ProductProps> = ({
    ...
    const [imagesFromRefinedProduct, setImagesFromRefinedProduct] = useState<ProductViewMedia[] | null>();
    ...
    const discount: boolean = refinedProduct
        ? refinedProduct.refineProduct?.priceRange?.minimum?.regular?.amount?.value >
          refinedProduct.refineProduct?.priceRange?.minimum?.final?.amount?.value
        : product?.price_range?.minimum_price?.regular_price?.value >
              product?.price_range?.minimum_price?.final_price?.value ||
          productView?.price?.regular?.amount?.value > productView?.price?.final?.amount?.value;
    ...
    const onProductClick = () => {
        window.magentoStorefrontEvents?.publish.searchProductClick(SEARCH_UNIT_ID, product?.sku);
    };
    ...
    const handleAddToCart = async () => {
        ...
        if (isSimple) {
            if (addToCart) {
                ...
                if (response?.errors || response?.data?.addProductsToCart?.user_errors.length > 0) {
                    setError(true);
                    return;
                }
        ...
        return (
            <>
                <div className="grid-container">
                    <div className={`product-image ds-sdk-product-item__image relative rounded-md overflow-hidden}`}>
                        <a
    ...
    return (
        ...
        >
                ...
                <div className="ds-sdk-product-item__main relative flex flex-col justify-between h-full">
                    <div className="ds-sdk-product-item__image relative w-full h-full rounded-md overflow-hidden">
                        {productImageArray.length ? (
                            ...
                            <NoImage className={`max-h-[45rem] w-full object-cover object-center lg:w-full`} />
                        )}
                    </div>
            ...
            <div className="pb-4 mt-sm">
                ...
                {isHovering && screenSize.desktop && <AddToCartButton onClick={handleAddToCart} />}
            </div>
        </div>
    );
};
